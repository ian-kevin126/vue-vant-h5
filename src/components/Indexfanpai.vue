<template>
  <div class="page indexPage">
    ...
    <div class="ab zp-con">
      ...
      <div
        class="ab zhuanlight"
        v-bind:style="{ top: lightpArr[nowlightIndex].top + 'px', left: lightpArr[nowlightIndex].left + 'px' }"
      >
        <img src="./../assets/images/p_zhuanlight.png" />
      </div>
      <div
        class="ab zhuanlight"
        v-bind:style="{ top: lightpArr[nowlightIndexa].top + 'px', left: lightpArr[nowlightIndexa].left + 'px' }"
      >
        <img src="./../assets/images/p_zhuanlight.png" />
      </div>
      <div
        class="ab zhuanlight"
        v-bind:style="{ top: lightpArr[nowlightIndexb].top + 'px', left: lightpArr[nowlightIndexb].left + 'px' }"
      >
        <img src="./../assets/images/p_zhuanlight.png" />
      </div>
    ...
    <div class="ab pais-con">
      <div
        ...
        :key="item.index"
        :id="index"
        v-bind:style="{ top: item.top + 'px', left: item.left + 'px' }"
      >
...
export default {
  ...
  methods: {
    ...
    downApp() {
      location.href =
        'https://static.codoon.com/html/cpm_download_codoon_app.html?id=https://www.codoon.com/activity/v1/play-over/index.html&codoon=activity'
    },
    resetDeg() {
        ...
        if (this.actInfo.is_nowprize) {
          this.prizedes_arr.forEach(element => {
            if (this.actInfo.nowprize_name.indexOf(element.pname) != -1) {
              this.zprotate('zpan', element.degnum, function () {
                _self.$store.dispatch('whickToast', { isShowToast: true, type: 2 })
              })
            }
          })
        } else {
          this.zprotate('zpan', 296, function () {
            _self.$store.dispatch('whickToast', { isShowToast: true, type: 1 })
          })
        }
      })
    },
...
//动画
.light_act_a {
  animation: animate_light_a 0.2s infinite linear;
}
...
.light_act_b {
  animation: animate_light_b 0.5s infinite linear;
}

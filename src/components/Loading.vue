...
export default {
  ...
  mounted() {
    ...
    if (len > 0) {
      list.forEach((item) => {
        const imgUrl = require('../assets/images/' + item)
    ...
    const bgUrl = require('../assets/images/logo.png')
...
.light_act_a {
  animation: animate_light_a 0.5s infinite linear;
}
